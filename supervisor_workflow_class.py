...
worker_model = ChatAnthropic(model="claude-3-5-sonnet-20241022", temperature=0)
supervisor_nodes = SupervisorNodes(worker_model)

ASSISTANT_WORKER_ID = "AssistantWorker"
ASSISTANT_WORKER_TASK = "introduces the agent and all the details about the agent (name, role and features)"
RESEARCHER_ID = "ResearcherWorker"
RESEARCHER_TASK = "searchs information about the user request related to the event COP16 and generates a response"
...
class SupervisorWorkflow:
    ...
    def __init__(self, has_researcher=True):
        ...
        # inject members
        self.workflow_members.update({ASSISTANT_WORKER_ID: ASSISTANT_WORKER_TASK})
        if has_researcher:
            self.workflow_members.update({RESEARCHER_ID: RESEARCHER_TASK})
    ...
    def gen_workflow(self):
        ...
        if self.has_researcher:
            process_request_crag_as_team_partial = partial(process_request_crag_as_team, RESEARCHER_ID)
            self.graph.add_node(RESEARCHER_ID, process_request_crag_as_team_partial)
            self.graph.add_edge(RESEARCHER_ID, "supervisor")
            self.action_map.update({RESEARCHER_ID:RESEARCHER_ID})
...
if __name__ == "__main__":
    ...
    message = " que planes tenes en metrallo??"
    reply = chain.invoke(
        message,
        {
            "recursion_limit": 150,
